...
public class WriteHbaseBolt extends BaseRichBolt {
	...
	/**
	 * class that write calculate result to hbase hbase map structure: row-key
	 */
	...
	public void writeHbase(Configuration conf, String rowKey, String columnFamlity, String column, String columnValue)
			throws IOException {
		/**
		 * this method used to write map to hbase hbase structure : row_key :
		 * channel_name#timestamp#code_name column family : "channel" column
		 * name : host_name value: code_count#toutal_count#ratio_count
		 */

		try {
			Put put = new Put(Bytes.toBytes(rowKey));
			put.add(Bytes.toBytes(columnFamlity), Bytes.toBytes(column), Bytes.toBytes(columnValue));
			table.put(put);
