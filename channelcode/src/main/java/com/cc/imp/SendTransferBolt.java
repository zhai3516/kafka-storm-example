...
public class SendTransferBolt extends BaseRichBolt {
	...
	/**
	 * class that send data to falcon-transfer for alarming
	 * data structure is same to the data write to opentsdb
	 */
...
//    		String reply = client.invokeAndReadResponse("Transfer.UpdateStrom", 
...
//    		logger.info(reply);		
	...
	private boolean initSocketConnection(String ip, int port, int retry){
		 ...
		 *  	ip: target ip
		 *  	port: target port
		 *  	retry: retry times if established faild.
		 *  		this parameter should between (0,10), or retry=10.
		 *    		default is 5.
		 */			
