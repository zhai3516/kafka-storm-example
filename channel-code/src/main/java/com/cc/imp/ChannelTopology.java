...
public class ChannelTopology {
	...
	public static void main(String[] args) throws Exception {
		...
		WriteOpenTSDBBolt writeTsdbBolt = new WriteOpenTSDBBolt(TopologyConfig.tsdbUrl, TopologyConfig.tsdbCountMetric,
				TopologyConfig.tsdbRatioMetric);
		...
		builder.setBolt(MESSAGE_SPLIT_BOLT_ID, splitBolt, TopologyConfig.splitBoltNum).shuffleGrouping(KAFKA_SPOUT_ID);
		...
		conf.setNumAckers(TopologyConfig.ackerNum);
	...
	public static void updateSettings(String[] args) {
		Option name = Option.builder("name").required(false).hasArg().desc("The name of storm topology").build();
		Option workers = Option.builder("workers").required(false).hasArg().desc("The num of storm workers").build();

		Option spountCount = Option.builder("spountCount").required(false).hasArg().desc("The num of spout tasks")
				.build();
		Option splitCount = Option.builder("splitCount").required(false).hasArg().desc("The num of split bolt tasks")
				.build();
		Option calCount = Option.builder("calCount").required(false).hasArg().desc("The num of calculate bolt tasks")
				.build();
		...
		Option kafkaTopic = Option.builder("topic").required(false).hasArg().desc("The topic of kafka").build();
		Option kafkaGroupId = Option.builder("group").required(false).hasArg().desc("The group id of kafka").build();
		Option kafkaZookeeperAddr = Option.builder("kafkazk").required(false).hasArg()
				.desc("The address of kafka zookeeper").build();

		Option tsdbuUrl = Option.builder("tsdburl").required(false).hasArg().desc("The url address of opentsdb")
				.build();
		Option tsdbCountMetric = Option.builder("count").required(false).hasArg().desc("The metric of count item")
				.build();
		Option tsdbRatioMetirc = Option.builder("ratio").required(false).hasArg().desc("The metric of ratio item")
				.build();

		Option hbaseZookeeperAddr = Option.builder("hbasezk").required(false).hasArg()
				.desc("The address of hbase zookeeper").build();
		Option hbaseMaster = Option.builder("master").required(false).hasArg().desc("The master address of hbase")
				.build();
		Option hbaseRootDir = Option.builder("rootdir").required(false).hasArg().desc("The root dir of hbase").build();

		Option transferIp = Option.builder("transferip").required(false).hasArg().desc("The ip of transfer").build();
		Option transferPort = Option.builder("transferport").required(false).hasArg().desc("The port of transfer")
				.build();
